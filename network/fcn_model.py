...
def loss(x,y,keep_probability):
    pred_annotation, logits = inference(x, 0.85)
    loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=tf.squeeze(logits), labels=tf.cast(y,tf.float32)))
